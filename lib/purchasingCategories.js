...
export function ensurePurchasingCategories(categories = [], cleanFn = (x) => x) {
  const inferred = (categories || []).map(inferPurchasingRole);
  const activePurchasing = inferred.filter(
    (c) => c?.role === "purchasing" && c?.type === "out" && c?.active !== false
  );
  if (activePurchasing.length >= 13) return cleanFn(inferred);
  ...
  for (const def of DEFAULT_PURCHASING_CATEGORIES) {
    ...
    if (!byName.has(key) || byName.get(key)?.role !== "purchasing") {
      byName.set(key, { ...def, ...(byName.get(key) || {}), role: "purchasing", type: "out", active: true });
    }
  }
