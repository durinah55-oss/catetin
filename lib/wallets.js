...
/** Nama lama sebelum migrasi Shopee/Grab Food — hanya ini yang di-rename otomatis. */
const LEGACY_FOOD_WALLET_NAMES = {
  w_pm: new Set(["payment method", "pm", "shopee pay", "shopeepay"]),
  w_nf: new Set(["night food", "nf", "grab pay", "grabpay"]),
};
...
export function patchWalletCatalog(wallets) {
  ...
  for (const [id, patch] of Object.entries(FOOD_WALLET_PATCH)) {
    const w = byId.get(id);
    if (!w) continue;
    const legacy = LEGACY_FOOD_WALLET_NAMES[id];
    const normalizedName = (w.name || "").trim().toLowerCase();
    if (legacy?.has(normalizedName)) {
