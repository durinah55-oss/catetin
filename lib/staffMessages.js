...
export function revisionStillPending(report, staffMessages, outlet) {
  ...
  if (msg?.meta?.fulfilledAt) return false;
...
export function createDailyReportSubmittedMessage({ report, author, resubmit = false }) {
  ...
  return {
    ...
    readBy: [],
    meta: { dailyReportId: report.id, reportDate: date, outlet, total: report.total, dedupeKey: `daily_report_submitted|${report.id}|${report.resubmittedAt || report.submittedAt || date}` },
  };
}
