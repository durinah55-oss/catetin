...
/** Kategori awal e-commerce / NF (bukan 13 kelompok purchasing resto). */
export const ECOMMERCE_DEFAULT_CATEGORIES = [
...
export function defaultCategoriesForBusiness(business) {
  ...
  if (type === "ecommerce" || type === "umkm") {
    return ECOMMERCE_DEFAULT_CATEGORIES.map((c) => ({ ...c }));
  }
  return [];
}
...
export function resolveCategoriesForBusiness(savedCats, business) {
  ...
  if (!list.length || hasFnBPurchasingCategories(list)) {
    return defaultCategoriesForBusiness(business) || [];
  }
  return list;
}
