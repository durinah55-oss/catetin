...
test("legacy ec categories replaced with NF catalog", () => {
  const fishing = { id: "fish-1", type: "ecommerce" };
  const legacy = [
    { id: "ec_out1", name: "Modal / Kulakan", type: "out", active: true },
    { id: "ec_in1", name: "Penjualan", type: "in", active: true },
  ];
  const resolved = resolveCategoriesForBusiness(legacy, fishing);
  assert.ok(resolved.some((c) => c.id === "nf_out_bahan"));
  assert.ok(resolved.some((c) => c.nfKind === "prive"));
});

test("detailed NF subcategories replaced with 12 besar", () => {
  const fishing = { id: "fish-1", type: "ecommerce" };
  const detailed = [
    { id: "nf_out_bahan_essen", name: "Bahan essen", type: "out", active: true },
    { id: "nf_out_kar_gaji", name: "Gaji", type: "out", active: true },
  ];
  const resolved = resolveCategoriesForBusiness(detailed, fishing);
  assert.equal(resolved.filter((c) => c.type === "out").length, 13);
  assert.ok(resolved.some((c) => c.id === "nf_out_bahan"));
  assert.ok(resolved.some((c) => c.id === "nf_out_kemasan"));
  assert.ok(!resolved.some((c) => c.id === "nf_out_bahan_essen"));
});

test("partial NF list merged to full 12 besar", () => {
  const fishing = { id: "fish-1", type: "ecommerce" };
  const partial = [
    { id: "nf_out_bahan", name: "Bahan produk", type: "out", active: true, nfKind: "hpp", purchasingUse: true },
    { id: "nf_out_produksi", name: "Produksi", type: "out", active: true, nfKind: "hpp", purchasingUse: true },
    { id: "nf_out_pengiriman", name: "Pengiriman", type: "out", active: true, nfKind: "opex", purchasingUse: true },
  ];
  const resolved = resolveCategoriesForBusiness(partial, fishing);
  assert.ok(resolved.some((c) => c.id === "nf_out_kemasan"));
  assert.ok(resolved.some((c) => c.id === "nf_out_marketing"));
});
test("NF purchasing hanya 4 kategori belanja (grup 1-4)", () => {
  const fishing = { id: "fish-1", type: "ecommerce" };
  const cats = resolveCategoriesForBusiness([], fishing);
  const purchasing = visibleCategoriesForBusiness(cats, { role: "purchasing" }, "out", fishing);
  assert.deepEqual(
    purchasing.map((c) => c.name).sort(),
    ["Bahan produk", "Kemasan", "Pengiriman", "Produksi"].sort()
  );
  assert.ok(!purchasing.some((c) => c.name === "Operasional"));
});
