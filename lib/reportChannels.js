...
/** KSM/SMT disamakan dengan template KBU; pertahankan override admin (active, order, settle). */
...
function resolveOutletChannelList(savedList, outlet) {
  if (outlet === "KBU") {
    return savedList?.length ? cloneChannels(savedList) : cloneChannels(KBU_REPORT_CHANNELS);
  }
  return syncToKbuFormTemplate(savedList);
}
