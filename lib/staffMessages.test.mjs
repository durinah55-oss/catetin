...
msg.createdAt = "2026-06-22T10:00:00.000Z";
...
const subMsg = createDailyReportSubmittedMessage({
  report: { id: "dr1", outlet: "KBU", date: "2026-06-21", total: 7486000 },
  author: { name: "Kasir KBU", role: "kasir" },
  resubmit: true,
});
ok(messageForUser(subMsg, { role: "admin" }), "admin terima notif laporan");
ok(messageForUser(subMsg, { role: "owner" }), "owner terima notif laporan admin-target");
...
ok(!revisionStillPending({ ...report, status: "admin_verified" }, [msg], "KBU"), "admin_verified tidak stuck revisi");
const resubmitted = {
  id: "dr1",
  outlet: "KBU",
  date: "2026-06-21",
...
const patched3 = applyRevisionNoticesFromMessages([stillRev], [msg]);
ok(patched3[0].status === "submitted", "applyRevision promote resubmitted revision_requested");
...
const ack = createRevisionSubmittedAckMessage({ report: resubmitted, author: { name: "Kasir KBU", id: "u_kasir" } });
ok(ack.kind === "revision_submitted" && ack.target.value === "KBU", "ack revisi ke outlet kasir");
ok(messageForUser(ack, { role: "kasir", outlet: "KBU", id: "u_kasir" }), "kasir KBU lihat ack di Pengumuman");
