 ...
 * Purchasing: hanya write-through dari bisnis aktif + milik sendiri.
 * Owner/admin: semua mutasi dompet sumber.
 */
export function filterSharedTransactionsForView(txs, { businessId, userId, role } = {}) {
  ...
  return (txs || []).filter((t) => {
    ...
    if (isPurchasing) {
      ...
      if (userId && meta.createdById && meta.createdById !== userId) return false;
    }
    return true;
  });
}
