...
/** Owner/admin keuangan hapus laporan belum settle + bersihkan transaksi omset tunai duplikat. */
export function deleteDailyReport(state, reportId, user) {
  if (!canDo(user?.role, "hapusLaporanOmset")) {
    throw new Error("Hanya owner atau admin keuangan yang bisa menghapus laporan omset kasir.");
  }
  const reports = state.dailyReports || [];
  const report = reports.find((r) => r.id === reportId);
  if (!report) throw new Error("Laporan tidak ditemukan.");
  if (report.status === "settled") {
    throw new Error("Laporan sudah disettle — tidak bisa dihapus.");
  }
  if (reportHasSettleTxs(reportId, state.transactions)) {
    throw new Error("Laporan sudah punya transaksi settle — tidak bisa dihapus.");
  }
  const removeIds = collectDailyReportCashTxIds(state.transactions, report);
