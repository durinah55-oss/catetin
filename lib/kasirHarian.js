...
export function submitDailyReport(state, payload) {
  ...
  const dup = reports.find(
    (r) => r.outlet === outlet && r.date === date && r.status !== "settled" && r.status !== "revision_requested"
  );
  if (dup) throw new Error(`Laporan ${date} untuk ${outlet} sudah dikirim.`);
...
export function resubmitDailyReport(state, reportId, payload) {
  ...
  if (report.status !== "revision_requested") throw new Error("Laporan ini tidak dalam status revisi.");
