...
import { isRevisionRequestMessage } from "./staffMessages.js";
...
export function reportAwaitingKasirRevision(report, staffMessages, outlet) {
  if (!report || report.status === "settled") return false;
  if (report.status === "revision_requested") return true;
  return (staffMessages || []).some(
    (m) =>
      isRevisionRequestMessage(m)
      && m.target?.value === outlet
      && (m.meta?.dailyReportId === report.id || m.meta?.reportDate === report.date)
  );
}
...
export function findPendingRevisionReport(reports, outlet, staffMessages) {
  ...
  const byStatus = (reports || [])
    .filter((r) => r.outlet === outlet && r.status === "revision_requested")
    .sort((a, b) => (b.date || "").localeCompare(a.date || ""));
  if (byStatus.length) return byStatus[0];
