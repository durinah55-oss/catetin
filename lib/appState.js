...
export function isDestructiveSave(remoteData, localPayload) {
  ...
  if (
    ...
    && local.dailyReports < remote.dailyReports - 1
    && local.transactions <= remote.transactions
  ) {
    return {
      blocked: true,
      reason: `laporan awan ${remote.dailyReports} vs lokal ${local.dailyReports}`,
    };
  }
...
function finalizeMergedDoc(merged) {
  ...
  return {
    ...merged,
    transactions: normalizeTransactions(dedupeTransactionsById(merged.transactions)),
  };
}
