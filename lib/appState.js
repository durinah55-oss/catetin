...
import { applyRevisionNoticesFromMessages } from "./staffMessages.js";
...
import { reconcileSettledLaciTransactions } from "./kasirHarian.js";
...
export { mergeDailyReports, pickNewerDailyReport } from "./dailyReportMerge.js";
...
export function mergeCategoriesFromDb(local = [], fromDb = []) {
  ...
  return ensurePurchasingCategories(dedupeCategories([...out.values()]), cleanCategoryList);
}
...
/**
 * Poll / refresh awan — transaksi & laporan omset mengikuti awan (bukan merge HP stale).
 * Dipakai setelah antrian simpan lokal selesai agar saldo tidak naik-turun dari data lama di HP.
 */
export function mergeAppStateFromCloudPull(remote, local) {
  ...
  const deletedTransactionIds = filterProtectedLaporanTxDeletes(
    mergeDeletedTransactionIds(remote.deletedTransactionIds, []),
    filterReports(mergeDailyReports(remote.dailyReports || [], [], { filterDeletedDailyReports: filterReports })),
    (remote.transactions || []).filter((t) => t?.id && !new Set(mergeDeletedTransactionIds(remote.deletedTransactionIds, [])).has(t.id))
  );
  const tomb = new Set(deletedTransactionIds);
  const cloudTxs = (remote.transactions || []).filter((t) => t?.id && !tomb.has(t.id));

  const mergedStaff = pruneStaleStaffMessages(
    mergeById(remote.staffMessages, []),
    filterReports(mergeDailyReports(remote.dailyReports || [], [], { filterDeletedDailyReports: filterReports }))
  );
  const cloudReports = applyRevisionNoticesFromMessages(
    mergeDailyReports(remote.dailyReports || [], [], {
...
export function mergeAppStateData(remote, local) {
  ...
  const mergedStaff = mergeById(remote.staffMessages, local.staffMessages);
  ...
  return finalizeMergedDoc({
    ...
    deletedDailyReportIds: deleteMeta.deletedDailyReportIds,
    deletedDailyReportSlots: deleteMeta.deletedDailyReportSlots,
    dailyReports: filterReports(mergedReports),
...
export async function saveAppState(bizId, data) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!error) return row?.updated_at || new Date().toISOString();
