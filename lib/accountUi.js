...
/** Tab bottom nav — Void HANYA kasir outlet. */
export function navConfig(user, _voidPending = 0) {
  ...
  if (showVoidTab(role)) right.push(["void", "Void"]);
  if (showAnalisisTab(role)) right.push(["analisis", "Analisis"]);
  if (showPurchasingAsistenTab(role)) right.push(["asisten", "Asisten"]);
  right.push(["profil", "Atur"]);

  return { left, right };
}

export function getAccountUi(user) {
  const role = user?.role || "kasir";
  ...
  if (role === "kasir" && outlet) {
    ...
    homeTitle = "NF3 · Owner";
    walletSectionTitle = "Seluruh dompet";
  }

  return {
    roleLabel,
    micTitle: role === "kasir" ? "Catat pengeluaran laci" : role === "purchasing" ? "Catat belanja" : "Catat transaksi",
    saldoGradient,
    homeTitle,
    homeSubtitle,
    walletSectionTitle,
