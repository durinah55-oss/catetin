...
import { normalizeTransaction, resolveWalletId } from "./transactionNormalize.js";
...
/** Riwayat penyesuaian manual — tampilkan sebelum owner tap lagi (cegah panik berulang). */
export function recentBalanceAdjustments(transactions, walletId, limit = 5) {
  return (transactions || [])
    .filter((t) => resolveWalletId(t) === walletId && t.source === "Sesuaikan Saldo")
    .sort((a, b) => (b.date || "").localeCompare(a.date || "") || String(b.id).localeCompare(String(a.id)))
    .slice(0, limit);
}

export function countBalanceAdjustments(transactions, walletId, sinceDateStr) {
  return (transactions || []).filter((t) => {
    if (resolveWalletId(t) !== walletId || t.source !== "Sesuaikan Saldo") return false;
    if (sinceDateStr && (t.date || "") < sinceDateStr) return false;
    return true;
  }).length;
}
