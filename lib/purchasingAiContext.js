...
import { isPurchasingTx, formatRupiah, purchasingOutletLabel } from "./purchasingExpense.js";
...
export function buildPurchasingAiContext({ transactions = [], categories = [], days = 30, outlet = "all" } = {}) {
  ...
  for (const t of txs) {
    ...
    bumpMap(byOutlet, purchasingOutletLabel(t.outlet), amt);
    ...
    for (const it of itemsDetailFromTx(t)) {
      ...
      if (!itemLast.has(it.name)) {
        itemLast.set(it.name, {
          ...
          date: t.date,
          supplier: t.supplier || "—",
          outlet: purchasingOutletLabel(t.outlet),
          txAmount: amt,
  ...
  return {
    ...
    outletFilter: outlet === "all" ? "Semua outlet" : purchasingOutletLabel(outlet),
    summary: {
